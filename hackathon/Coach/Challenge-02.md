...
1. Update `pom.xml`:
   ...
   - Add `spring-cloud-azure-starter-storage-blob` (or use the Azure SDK directly)
...
4. Migrate photo upload/retrieval from Oracle BLOB → Azure Blob Storage:
   - `PhotoService.java` (or equivalent): use `BlobServiceClient` to upload/download bytes
...
| Azure Blob Storage credentials during local dev | Suggest using the Azurite emulator (`docker run mcr.microsoft.com/azure-storage/azurite`) or a real Azure account |
| Build fails due to Hibernate 6 breaking changes | Spring Boot 3 ships with Hibernate 6, which has different behaviour for `@GeneratedValue`. Ask Copilot Chat to explain the errors. |
