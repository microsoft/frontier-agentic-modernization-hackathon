# Coach Guide – Challenge 06: Migrate the Database to Azure (.NET Track)

## Purpose

This challenge takes the squad through a **production-grade data migration** from an on-premises SQL Server (LocalDB / SQL Express) to the Azure SQL Database that was provisioned in Challenge 03. Students use **Azure Database Migration Service (DMS)** — operated directly from the Azure Portal — to assess compatibility, migrate the schema, copy data, and perform cutover validation. This is the real-world capstone step: the modernized application now runs entirely on Azure with its data in a managed, geo-redundant cloud database.
