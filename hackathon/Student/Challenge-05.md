...
# Challenge 05 – Observe, Validate & Secure (Stretch)
...
This stretch challenge pushes the modernized applications toward production readiness using three Azure capabilities:
...
Apply production hardening to both modernized applications:

**Observability:**
- Integrate the Application Insights SDK into both the Java and .NET applications
- Verify that HTTP requests, dependency calls (database, blob storage, service bus), and exceptions appear in the Application Insights portal

**Secret management:**
- Provision an Azure Key Vault and store all connection strings and credentials as secrets
- Configure both Container Apps to use a **User-Assigned Managed Identity** to access Key Vault — no connection strings in `appsettings.json`, `application.properties`, or Terraform variable files
...
**CI/CD pipeline:**
- Review (or create) a GitHub Actions workflow that builds, tests, and deploys both applications on push to the `main` branch
- The pipeline should run `modernize assess` as a quality gate and fail the build if critical issues are found

> **Hint:** The Azure SDK for both Java and .NET supports `DefaultAzureCredential`, which transparently uses Managed Identity when running in Azure and developer credentials locally.
...
- Application Insights shows live telemetry (requests, dependencies, exceptions) from both applications
...
- Both Container Apps use Managed Identity (confirm in Azure Portal → Container App → Identity)
